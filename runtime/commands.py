...
class Commands(object):
    ...
    def __init__(self, conffile=None, verify=False, no_patch=False, workdir=None, json=None, shortstart=False):
        ...
        normal_start = self.readconf(workdir, json)
        ...
        if not normal_start and not shortstart:
    ...
    def readconf(self, workdir, json):
        ...
        if set(['notch', 'name', 'package', 'side']) <= header_classes and \
                set(['notch', 'searge', 'notchsig', 'sig', 'classnotch', 'classname', 'package',
                     'side']) <= header_methods and \
        ...
        self.versionClient, self.versionServer = self.get_versions()
    ...
    def checkjars(self, side):
        ...
        if side == CLIENT:
            ...
            if fail:
                self.logger.error(
                    '!! Libraries check FAILED. Make sure to copy the entire .minecraft/libraries folder into jars !!')
                sys.exit(1)
        return True
    ...
    def applyss(self, side, reobf=False, srg_names=False, in_jar=None, out_jar=None, keep_lvt=False,
                keep_generics=False):
    ...
    def createclssrg(self, side):
        ...
        for line in log:
            ...
            if '=CL_00' in line:
                columns = line.split("=")
                renames[columns[0]] = columns[
                                          1] + '_'  # This is added here just to make sure our numbers don't run on to anything else
    ...
    def applyexceptor(self, side, exc_update=False, dryrun=False):
        ...
        if not dryrun:
            ...
            forkcmd = self.cmdexceptordry.format(input=excinputdry[side], conf=excconf[side], log=exclogdry[side],
                                                 json=json[side])
    ...
    def recompile(self, side):
        ...
        if self.cmdrecompscala:  # Compile scala before java as scala scans java files, but not vice-versa
            ...
            pkglistscala.extend(
                filterdirs(pathsrclk[side], '*.scala', append_pattern=append_pattern, all_files=all_files))
            dirs = ' '.join(pkglistscala)
            classpath = os.pathsep.join(cplk[side])
            forkcmd = self.cmdrecompscala.format(classpath=classpath, sourcepath=pathsrclk[side],
                                                 outpath=pathbinlk[side], pkgs=dirs)
            try:
    ...
    def startclient(self, mainclass, extraargs):
        ...
        os.chdir(self.dirjars)
        forkcmd = self.cmdstartclt.format(classpath=classpath, natives=natives, mainclass=mainclass,
                                          extraargs=extraargs)
        self.runmc(forkcmd)
            ...
            for in_class in trgclasses:
                ...
                if rename:
                    try:
                        ...
                        f.close()
                        self.logger.info('> Outputted %s to %s as %s', in_class.ljust(35), outpathlk[side],
                                         '_' + out_class)
                    except IOError:
    ...
    def downloadupdates(self, force=False):
        ...
        if 'CHANGELOG' in [i[0] for i in newfiles]:
            ...
            changelog = urlopen(changelog_url).readlines()
